...
interface InteractiveBaseLogoProps {
  className?: string
}

const randomColors = [
  '#FF00FF', '#00FFFF', '#00FF00', '#FFFF00', '#FF6600', '#FF0080', 
  '#8000FF', '#FF4500', '#32CD32', '#FF1493', '#00CED1', '#FFD700',
  '#DC143C', '#9932CC', '#FF8C00', '#00FA9A', '#FF69B4', '#00BFFF'
];

const getRandomColor = () => randomColors[Math.floor(Math.random() * randomColors.length)];

export function InteractiveBaseLogo({ className }: InteractiveBaseLogoProps) {
  ...
  const handleBoxHover = (boxNumber: number) => {
    ...
    setBoxColors(prev => ({
      ...prev,
      [`box${boxNumber}`]: randomColor
    }));
  };

  const handleBoxLeave = (boxNumber: number) => {
    setBoxColors(prev => ({
      ...prev,
      [`box${boxNumber}`]: '#0000FF'
    }));
  };

  return (
    <div className={cn('flex justify-center interactive-base-logo', className)}>
      <svg 
      ...
      >
        {/* Box 1 - B */}
        <path 
          className="interactive-base-box-1"
          onMouseEnter={() => handleBoxHover(1)}
          onMouseLeave={() => handleBoxLeave(1)}
          onTouchStart={() => handleBoxHover(1)}
          onTouchEnd={() => handleBoxLeave(1)}
        ...
        {/* Box 2 - A */}
        <path 
          className="interactive-base-box-2"
          onMouseEnter={() => handleBoxHover(2)}
          onMouseLeave={() => handleBoxLeave(2)}
          onTouchStart={() => handleBoxHover(2)}
          onTouchEnd={() => handleBoxLeave(2)}
        ...
        {/* Box 3 - S */}
        <path 
          className="interactive-base-box-3"
          onMouseEnter={() => handleBoxHover(3)}
          onMouseLeave={() => handleBoxLeave(3)}
          onTouchStart={() => handleBoxHover(3)}
          onTouchEnd={() => handleBoxLeave(3)}
        ...
        {/* Box 4 - E */}
        <path 
          className="interactive-base-box-4"
          onMouseEnter={() => handleBoxHover(4)}
          onMouseLeave={() => handleBoxLeave(4)}
          onTouchStart={() => handleBoxHover(4)}
          onTouchEnd={() => handleBoxLeave(4)}
