...
function BaseExpandedLoadingLogo({ className, text, size = 'md' }: BaseExpandedLoadingLogoProps) {
  ...
  const [boxColors, setBoxColors] = useState<Record<'box1' | 'box2' | 'box3' | 'box4', string>>({
    box1: RANDOM_COLORS[0],
    box2: RANDOM_COLORS[1],
    box3: RANDOM_COLORS[4],
    box4: RANDOM_COLORS[6],
  });
  ...
  useEffect(() => {
    ...
    const syncColorAnimation = () => {
      stopColorAnimation();
      if (performanceModeEnabled || reducedMotion.matches) return;
      initialTimeout = setTimeout(changeColors, 100);
      interval = setInterval(changeColors, 1500);
    };
