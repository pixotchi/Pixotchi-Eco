    ...
    return (
        <EventWrapper event={event} shopItemMap={shopItemMap} gardenItemMap={gardenItemMap}>
            <p className="text-sm">
                ...
                <span className="font-semibold text-primary">{formattedScore}</span>
                {' PTS!'}
            </p>
        </EventWrapper>
    );
...
export const QuestFinalizedEventRenderer = ({ event, userAddress }: { event: QuestFinalizedEvent, userAddress?: string | null }) => {
  ...
  return (
    <EventWrapper event={event}>
      <p className="text-sm">
        <span className="font-bold">Land #{event.landId}</span> completed a quest and earned <span className="font-semibold text-primary">{reward}</span>.
      </p>
    </EventWrapper>
  );
};
