    ...
    {rows.map(row => <div key={row.label} className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
      ...
      <dd className="min-w-0 font-medium tabular-nums [overflow-wrap:anywhere]">
        <ResourceValue resource={row.kind === 'points' ? 'points' : 'lifetime'}>
          {row.kind === 'points' ? <TokenAmount animated={row.label === "Stored plant points"} amount={row.amount} decimals={12} unit="PTS" withIcon={false} /> : <span title={formatDurationSeconds(row.amount, 'exact')}>{formatDurationSeconds(row.amount)}</span>}
        </ResourceValue>
      </dd>
    </div>)}
  </dl>;
