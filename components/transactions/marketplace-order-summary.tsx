...
export function MarketplaceOrderSummary({ order }: { order: MarketplaceOrder }) {
  ...
  return <div className="min-w-0 space-y-1 break-all text-xs leading-relaxed">
    ...
    <p>You give <strong className="tabular-nums">{formatUnits(order.amountAsk, 18)} {giveToken}</strong></p>
    <p>You receive <strong className="tabular-nums">{formatUnits(order.amount, 18)} {receiveToken}</strong></p>
