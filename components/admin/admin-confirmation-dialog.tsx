...
export function AdminConfirmationDialog({ state, onClose }: { state: AdminConfirmationState; onClose: () => void }) {
  ...
  return <Dialog open={state.open} onOpenChange={(open) => { if (!open) onClose(); }}>
    <DialogContent layout="form" size="md">
      ...
      <DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button variant={state.isDangerous ? 'destructive' : 'default'} disabled={!matches} onClick={() => { if (matches) { state.onConfirm(); onClose(); } }}>{state.confirmText}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
