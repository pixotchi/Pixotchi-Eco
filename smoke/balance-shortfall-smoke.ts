...
assert.equal(getBalanceShortfallMessage(parseUnits('499.999999999999999999', 18), parseUnits('500', 18), 'SEED'), 'Not enough SEED. You need less than 0.01 more.');
assert.equal(getBalanceShortfallMessage(BigInt(0), BigInt(1), 'ETH'), 'You need less than 0.000001 ETH.');
assert.equal(getBalanceShortfallMessage(BigInt(1_234_567), BigInt(2_000_000), 'usdc', 6), 'Not enough USDC. You need 0.77 more.');
...
assert.equal(getBalanceShortfallMessage(parseUnits('1', 18), parseUnits('200818.364981253622716697', 18), 'LEAF'), 'Not enough LEAF. You need 200,817.37 more.');
