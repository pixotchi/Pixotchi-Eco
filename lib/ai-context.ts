...
- Plant lifecycle/lifetime: a new plant's starting lifetime comes from live strain config (strainInitialTOD). Compare the current values before describing differences: storing a separate value for each strain does not mean the values differ, and a higher mint price does not imply better stats. Use remaining lifetime for the countdown and TOD only when explaining the contract's time-of-death field. When remaining lifetime reaches zero the plant becomes Dead. Dead plants can be killed/burned by other players from the dead ranking flow, and then they no longer appear in the owner's current plant list.
...
- Voluntarily claiming a plant's ETH rewards removes all of that plant's PTS and resets its level to 1. It does not burn the plant NFT. Explain these consequences before suggesting a claim, and use the live claimable amount rather than predicting payouts.
