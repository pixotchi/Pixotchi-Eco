...
export const KNOWLEDGE_TOPICS: Record<KnowledgeTopicId, KnowledgeTopicRecord> = {
  ...
  arcade: {
    ...
    canRead: ['Plant star balances through plant reads.', 'Recent public arcade/spin activity when indexed.'],
    ...
    id: 'arcade',
    liveDataSources: ['get_plants', 'get_wallet_game_activity', 'get_activity'],
  ...
  barracks_raids: {
    ...
    canRead: ['Owned lands.', 'Barracks built state.', 'Troop state exposed by land reads.', 'Recent raid activity.'],
    ...
    id: 'barracks_raids',
    liveDataSources: ['get_lands', 'get_wallet_game_activity', 'get_activity'],
  ...
  bridge_solana: {
    ...
    canRead: ['General bridge guidance.', 'Public status or transaction result when a Base tx hash is provided.'],
    ...
    id: 'bridge_solana',
    liveDataSources: ['get_transaction_status'],
  ...
  buildings: {
    ...
    canRead: ['Owned lands.', 'Town buildings.', 'Village buildings.', 'Building levels.', 'Production rates.', 'Unclaimed production.'],
    ...
    id: 'buildings',
    liveDataSources: ['get_lands', 'get_wallet_token_balances', 'get_wallet_game_activity'],
  ...
  casino: {
    ...
    canRead: ['Casino built status through land reads.', 'Recent public casino activity when indexed.'],
  ...
  lands: {
    ...
    canRead: ['Owned lands.', 'Public land IDs.', 'Land buildings.', 'Warehouse balances.', 'Production totals.', 'Quest slots.', 'Barracks state.'],
    ...
    id: 'lands',
    liveDataSources: ['get_lands', 'get_wallet_game_assets', 'get_wallet_game_activity'],
  ...
  marketplace: {
    ...
    canRead: ['Land ownership and building state.', 'General marketplace guidance.'],
    ...
    id: 'marketplace',
    liveDataSources: ['get_lands', 'get_wallet_token_balances'],
    purpose: 'Use land marketplace/orderbook flows where the Marketplace building is available.',
    stalenessRules: ['Orderbook state is not currently exposed to Neural Seed as a live read tool.'],
  ...
  mint_lands: {
    ...
    canRead: ['Live land mint price.', 'User SEED balance.', 'Current owned land count.', 'Recent land mint activity.'],
    ...
    id: 'mint_lands',
    liveDataSources: ['get_game_prices', 'get_wallet_token_balances', 'get_wallet_game_assets', 'get_wallet_game_activity'],
  ...
  mint_plants: {
    ...
    canRead: ['Live strain list.', 'Mint prices with payment token labels.', 'Remaining supply.', 'User token balances.', 'Owned plants.', 'Recent plant mint activity.'],
  ...
  missions: {
    ...
    canRead: ['Authenticated mission day.', 'Mission score.', 'Current and best streak.', 'Public mission/Rocks leaderboard.'],
    ...
    id: 'missions',
    liveDataSources: ['get_missions', 'get_leaderboards', 'get_wallet_game_activity'],
    ...
    userFlows: ['Open the Tasks button in the status bar.', 'Read Farmer\'s Tasks for today\'s Rock progress and streak.', 'Complete visible sections: General, Social, Land, and Plant.', 'Land tasks include Apply resources, Send a farmer on a quest, Claim/Collect production, and Play casino.', 'Plant tasks include buying elements, buying a shield/fence, collecting a star by killing a plant, and playing Arcade.', 'Wait for proof indexing where required, then re-open Tasks to check progress.'],
  ...
  onboarding: {
    ...
    id: 'onboarding',
    liveDataSources: ['get_player_overview', 'get_game_prices', 'get_wallet_token_balances', 'get_game_action_guide'],
  ...
  plant_attack: {
    aliases: ['attack', 'kill plant', 'combat', 'target', 'fence', 'shield'],
    canRead: ['Owned plants.', 'Public plant leaderboard entries.', 'Plant protection/fence state.', 'Recent attack activity.'],
    ...
    id: 'plant_attack',
    liveDataSources: ['get_plants', 'get_leaderboards', 'get_wallet_game_activity', 'get_activity'],
    purpose: 'Attack eligible plants from the ranking/farm combat flow.',
    stalenessRules: ['Attack availability depends on live cooldowns, protection, plant state, and UI rules.'],
    title: 'Plant Attacks',
  ...
  plant_care: {
    ...
    canRead: ['Owned plants.', 'Plant status.', 'TOD/time until starving.', 'PTS.', 'Stars.', 'Fences.', 'Active items.', 'Shop/garden prices.'],
    ...
    id: 'plant_care',
    liveDataSources: ['get_player_overview', 'get_plants', 'get_game_prices', 'get_wallet_game_activity'],
  ...
  prices: {
    ...
    canRead: ['Live strain mint prices.', 'Land mint price.', 'Revive price.', 'Shop item prices.', 'Garden item prices.', 'Fence quote.'],
    cannotDo: ['Predict future prices, give investment advice, or infer prices not returned by tools.'],
    deferralText: 'If a price is unavailable, check the exact panel in the app before acting.',
    id: 'prices',
    liveDataSources: ['get_game_prices', 'get_swap_quote'],
    purpose: 'Answer live price and cost questions using exact priceDisplay labels.',
    stalenessRules: ['Treat prices, payment tokens, ratios, quotes, and availability as live values.'],
  ...
  quests: {
    ...
    canRead: ['Owned lands.', 'Quest slots exposed by land reads.', 'Mission task status.', 'Recent quest activity.'],
    ...
    id: 'quests',
    liveDataSources: ['get_lands', 'get_wallet_game_activity', 'get_missions'],
  ...
  staking: {
    ...
    canRead: ['Staked SEED.', 'Claimable LEAF rewards.', 'Total staked.', 'Reward ratio.', 'Allowance/read-only approval status.'],
    cannotDo: ['Approve, stake, unstake, or claim LEAF.'],
    deferralText: 'Open Status bar -> Stake or Stake House for live staking actions.',
    id: 'staking',
    liveDataSources: ['get_staking', 'get_wallet_token_balances', 'get_wallet_game_activity'],
  ...
  support: {
    ...
    canRead: ['General troubleshooting guidance.', 'Visible status/check data when available through app status tools.'],
  ...
  swap: {
    ...
    canRead: ['Read-only informational swap quotes for supported tokens.', 'User balances through wallet tools.'],
    cannotDo: ['Execute swaps, approve tokens, prepare transactions, or give financial advice.'],
    deferralText: 'Open Swap and fetch a fresh quote before taking action.',
    id: 'swap',
    liveDataSources: ['get_swap_quote', 'get_wallet_token_balances'],
  ...
  verify_airdrop: {
    ...
    canRead: ['General guidance only unless a safe status tool is added.'],
    ...
    stalenessRules: ['Eligibility and campaign availability must come from visible UI or a future safe status tool.'],
  ...
  wallet: {
    ...
    canRead: ['Authenticated public wallet address.', 'Known token balances.', 'Owned game NFTs.', 'Public onchain/indexed wallet activity.', 'Transaction receipt status for a tx hash.'],
    ...
    id: 'wallet',
    liveDataSources: ['get_wallet_token_balances', 'get_wallet_game_assets', 'get_wallet_game_activity', 'get_transaction_status'],
  ...
  warehouse: {
    ...
    id: 'warehouse',
    liveDataSources: ['get_lands', 'get_wallet_game_activity'],
