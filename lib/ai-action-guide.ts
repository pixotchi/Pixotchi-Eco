...
export const KNOWLEDGE_TOPICS: Record<KnowledgeTopicId, KnowledgeTopicRecord> = {
  ...
  casino: {
    ...
    cannotDo: ['Place bets, reveal bets, approve betting tokens, fetch private random seeds, reveal casino custody wallet balances/transfers, or predict outcomes.'],
  ...
  notifications: {
    ...
    canRead: ['Public notification provider status.', 'Plant-care reminder service stats.', 'Current app notification provider label.', 'Feature/status caveats.'],
    cannotDo: ['Send notifications, add users to campaigns, read admin campaign data, force opt-in users, or reveal notification API keys.'],
    ...
    stalenessRules: ['Notification delivery depends on provider opt-in, provider health, and reminder throttling.', 'Plant-care reminders are best-effort and can lag if the provider or status service is degraded.'],
