export const SEED_PAIR_ADDRESS = '0xaa6a81a7df94dab346e2d677225cad47220540c5';
export const SEED_PAIR_DEXSCREENER_URL = `https://dexscreener.com/base/${SEED_PAIR_ADDRESS}`;
export const SEED_PAIR_DEXSCREENER_API = `https://api.dexscreener.com/latest/dex/pairs/base/${SEED_PAIR_ADDRESS}`;

const CACHE_DURATION = 5 * 60 * 1000;

export type MarketWindow = 'm5' | 'h1' | 'h6' | 'h24';
export type DexPairWindowStats = Partial<Record<MarketWindow, number>>;
export type DexTxnStats = Partial<Record<MarketWindow, { buys?: number; sells?: number }>>;

export type SeedMarketData = {
  cached?: boolean;
  dexId?: string;
  fdv: number | null;
  liquidityBase: number | null;
  liquidityQuote: number | null;
  liquidityUsd: number | null;
  marketCap: number | null;
  pairAddress?: string;
  pairCreatedAt: number | null;
  pairUrl?: string;
  priceChange: DexPairWindowStats;
  priceNative: string | null;
  priceUsd: string | null;
  rewards: number;
  stale?: boolean;
  timestamp: number;
  txns: DexTxnStats;
  volume: DexPairWindowStats;
  volume24h: number;
};

type DexScreenerPair = {
  dexId?: string;
  fdv?: number | null;
  liquidity?: {
    base?: number | null;
    quote?: number | null;
    usd?: number | null;
  } | null;
  marketCap?: number | null;
  pairAddress?: string;
  pairCreatedAt?: number | null;
  priceChange?: DexPairWindowStats | null;
  priceNative?: string | null;
  priceUsd?: string | null;
  txns?: DexTxnStats | null;
  url?: string;
  volume?: DexPairWindowStats | null;
};

let cachedData: SeedMarketData | null = null;

function normalizeNumber(value: unknown): number | null {
  return typeof value === 'number' && Number.isFinite(value) ? value : null;
}

function normalizeWindowStats(value: unknown): DexPairWindowStats {
  const source = value && typeof value === 'object' ? value as Record<string, unknown> : {};
  return {
    ...(normalizeNumber(source.m5) !== null ? { m5: normalizeNumber(source.m5)! } : {}),
    ...(normalizeNumber(source.h1) !== null ? { h1: normalizeNumber(source.h1)! } : {}),
    ...(normalizeNumber(source.h6) !== null ? { h6: normalizeNumber(source.h6)! } : {}),
    ...(normalizeNumber(source.h24) !== null ? { h24: normalizeNumber(source.h24)! } : {}),
  };
}

function normalizeTxns(value: unknown): DexTxnStats {
  const source = value && typeof value === 'object' ? value as Record<string, unknown> : {};
  const result: DexTxnStats = {};

  (['m5', 'h1', 'h6', 'h24'] as const).forEach((window) => {
    const entry = source[window];
    if (!entry || typeof entry !== 'object') {
      return;
    }

    const record = entry as Record<string, unknown>;
    const buys = normalizeNumber(record.buys);
    const sells = normalizeNumber(record.sells);
    result[window] = {
      ...(buys !== null ? { buys } : {}),
      ...(sells !== null ? { sells } : {}),
    };
  });

  return result;
}

export async function fetchSeedMarketPulse(): Promise<SeedMarketData> {
  const now = Date.now();
  if (cachedData && (now - cachedData.timestamp) < CACHE_DURATION) {
    return {
      ...cachedData,
      cached: true,
    };
  }

  try {
    const response = await fetch(SEED_PAIR_DEXSCREENER_API, {
      headers: {
        Accept: 'application/json',
      },
      next: { revalidate: 300 },
    });

    if (!response.ok) {
      throw new Error(`DexScreener API error: ${response.status}`);
    }

    const data = await response.json() as Record<string, unknown>;
    const pair = ((data?.pair ?? (Array.isArray(data?.pairs) ? data.pairs[0] : undefined)) ?? {}) as DexScreenerPair;
    const volume = normalizeWindowStats(pair.volume);
    const priceChange = normalizeWindowStats(pair.priceChange);
    const txns = normalizeTxns(pair.txns);
    const volume24h = volume.h24 ?? 0;

    cachedData = {
      dexId: pair.dexId,
      fdv: normalizeNumber(pair.fdv),
      liquidityBase: normalizeNumber(pair.liquidity?.base),
      liquidityQuote: normalizeNumber(pair.liquidity?.quote),
      liquidityUsd: normalizeNumber(pair.liquidity?.usd),
      marketCap: normalizeNumber(pair.marketCap),
      pairAddress: pair.pairAddress,
      pairCreatedAt: normalizeNumber(pair.pairCreatedAt),
      pairUrl: pair.url,
      priceChange,
      priceNative: typeof pair.priceNative === 'string' ? pair.priceNative : null,
      priceUsd: typeof pair.priceUsd === 'string' ? pair.priceUsd : null,
      rewards: volume24h * 0.02,
      timestamp: now,
      txns,
      volume,
      volume24h,
    };

    return {
      ...cachedData,
      cached: false,
    };
  } catch (error) {
    if (cachedData) {
      return {
        ...cachedData,
        cached: true,
        stale: true,
      };
    }

    throw error;
  }
}
